...
import { AZURE_STORAGE_ACCOUNT_NAME, AZURE_STORAGE_ACCOUNT_KEY, AZURE_STORAGE_AVATAR_BACKET } from '../../config/index.js';
...
export const uploadToAzure = async (file) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const containerClient = blobServiceClient.getContainerClient(AZURE_STORAGE_AVATAR_BACKET);

            // Создаем контейнер, если его еще нет
            if (!(await containerClient.exists())) {
                await containerClient.create();
                console.log(`Контейнер "${AZURE_STORAGE_AVATAR_BACKET}" создан`);
            }
